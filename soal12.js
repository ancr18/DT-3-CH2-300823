...
function getData(data){
    ...
    // 3. hitung data satu persatu
    data.forEach(user => {

        // 4. buat variabel untuk menampung data yang dibutuhkan
        const name = user.name
        const age = user.age 
        const gender = user.gender
        const eyeColor = user.eyeColor;

        // 5. buat kondisi untuk kriteria yang telah ditentukan
        if((name.length < 10 && age > 30) || (gender === 'male' && eyeColor == 'brown ')){
            result.push(user)
        }
...
console.log(`Jumlah user yang mempunyai nama kurang dari 10 karakter dan umur nya diatas 30 atau gender nya Male dan eyeColor nya brown adalah : ${getData(users).length}`)